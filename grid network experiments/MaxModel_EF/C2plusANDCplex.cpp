...
/*
*Maximum model
*Extended Formulation (EF) of subproblem
...
int main() {
	...
	string output_name = "ResultsMAX_thread4_all.csv";
	...
	for (size_t allDatasets = 0; allDatasets < 80; allDatasets++)
	{
		...
		while (flag != 1)
		{
			...
			if (MIP_needed == 1)
			{
				...
				typedef IloArray<IloNumVarArray> MatrixMinus1;
				MatrixMinus1 BETAminus1(env, activeArcs.size());
				typedef IloArray<IloNumVarArray> Matrixplus1;
				Matrixplus1 BETAplus1(env, activeArcs.size());

				typedef IloArray<IloNumVarArray> MatrixMinus2;
				MatrixMinus2 BETAminus2(env, activeArcs.size());
				typedef IloArray<IloNumVarArray> Matrixplus2;
				Matrixplus2 BETAplus2(env, activeArcs.size());

				for (size_t i = 0; i < activeArcs.size(); i++) {
					BETAminus1[i] = IloNumVarArray(env, NumberOfAttacks, 0, IloInfinity);
					BETAplus1[i] = IloNumVarArray(env, NumberOfAttacks, 0, IloInfinity);
					BETAminus2[i] = IloNumVarArray(env, NumberOfAttacks, 0, IloInfinity);
					BETAplus2[i] = IloNumVarArray(env, NumberOfAttacks, 0, IloInfinity);
				}

				IloNumVarArray Alpha1(env, NumberOfAttacks);
				IloNumVarArray Alpha2(env, NumberOfAttacks);
				for (size_t i = 0; i < NumberOfAttacks; i++) {
					Alpha1[i] = IloNumVar(env, 0);
					Alpha2[i] = IloNumVar(env, 0);
				}
				...
				for (size_t i = 0; i < activeArcs.size(); i++)
				{
					...
					for (size_t t = 0; t < NumberOfAttacks; t++)
					{
						model.add(p1a[activeArcs[i]] + BETAminus1[i][t] - BETAplus1[i][t] == Alpha1[t]); // Constraint 4c ------------------------
						model.add(p2a[activeArcs[i]] + BETAminus2[i][t] - BETAplus2[i][t] == Alpha2[t]); // Constraint 4d ------------------------
					}

					for (size_t t = 0; t < NumberOfAttacks; t++)
						for (size_t j = 0; j < k[t]; j++) // Constraint 4e ------------------------
							model.add((BETAminus1[i][t] + BETAminus2[i][t] + BETAplus1[i][t] + BETAplus2[i][t]) - BIGvalue[activeArcs[i]] * (1 - Lambda[i][t][j]) <= q[t][j + 1]);
					...
					//TIME OF SOLVING-------------------------------------------------------------
					/*MIP_end = clock();
					double elapsed_secs_MIP = double(MIP_end - MIP_begin) / CLOCKS_PER_SEC;*/
					...
					for (size_t t = 0; t < NumberOfAttacks; t++)
					{
						Alp1[t] = cplex0.getValue(Alpha1[t]);
						Alp2[t] = cplex0.getValue(Alpha2[t]);
